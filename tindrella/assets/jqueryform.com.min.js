...
! function() {
    ...
    function d(a) {
        ...
        var e = function(a, b) {
            ...
            f = function() {
                var a, b = function() {
                        var a = function() {
                                $("#g-recaptcha-response").length && ($("#g-recaptcha-response").rules("add", {
                                    required: !0
                                }), clearInterval(b))
                            },
                            b = setInterval(a, 800)
            ...
            j = function() {
                var a = $(".jf-thankyou");
                if (a.length) {
                    var b = $.trim(a.data("redirect"));
                    b = decodeURIComponent(b);
                    var c = i(a.html());
                    if (a.html(c), "" === b) return void a.show("slow");
                    var d = parseInt(a.data("seconds"));
                    d > 0 ? a.show("slow") : d = 0, setTimeout(function() {
                        b = i(b);
                        try {
                            parent && (parent.location.href = b)
                        } catch (a) {
                            location.href = b
                        }
                    }, 1e3 * d)
            ...
            l = function(a) {
                ...
                $(a).find("div.required").not(".jf-hide,.jf-disabled").each(function() {
                    b.push($(this).data("cid"))
                }), $("#serverValidationFields").val(b.join(",")), $(".submit p.error").removeClass("server-error"), $btn = new k, $btn.showProgressBar();
                var c = function() {
                    var b = $(a).ajaxSubmit(),
                        c = b.data("jqxhr");
                    c.done(function(a) {
                        a = $.parseJSON(a), a && a.validated === !0 ? ($(".jf-form").hide(), h = a.fieldValues, j()) : ($(".submit p.error").addClass("server-error").show("slow"), $btn.hideProgressBar())
                    })
                };
                setTimeout(c(), 3e3)
            };
        ...
        var m = function() {
                var a = $(this);
                return a.hasClass("g-recaptcha-response") ? !1 : (yes = a.is(":hidden") || a.is(":input.tmp-disabled"), yes)
            },
            n = 0 === String($(a).data("licensekey")).toUpperCase().indexOf("JF-");
        n ? $(".jf-copyright").remove() : (c(), setInterval(c, 1e4)), $(a).validate({
    ...
    function e(a) {
        $.jMaskGlobals.watchDataMask = !0, $(a).each(function(a, b) {
            ...
            }), e.datepicker(d), e.next(".input-group-addon.right").click(function() {
                e.datepicker("show")
            })
        })
    }
    ...
    function f() {
        $("input[type=file].form-control").each(function(a, b) {
            var c = {
                ...
                d = $(b),
                e = d.data("allowedfileextensions"),
                f = d.data("maxfilesize");
            e && (c.allowedFileExtensions = String(e).replace(/\s+/g, "").split(",")), f && (c.maxFileSize = parseFloat(String(f))), d.fileinput(c)
        }), setTimeout(function() {
        ...
        if (c) {
            ...
            $(g).change(function() {
                ...
                for (var g = 0, h = c.length; h > g; g++) {
                    var i = c[g];
                    d.push('<option value="' + escape(i.value) + '">' + i.label + "</option>")
                }
                $(f).append(d.join())
            })
        }
...
function(a) {
    ...
    var b = function(b) {
        ...
        d = function(d) {
            this.disabled = d && d.disabled || !1, this.action = d && d.action || "", this.selector = d && d.selector || "", this.match = d && d.match || "", this.rules = d && d.rules || [];
            var e = this,
                f = !1,
                g = new b(e.selector),
                h = g.$field,
                ...
                k = {
                    all: function() {
                        for (var a = !0, b = 0, c = j.length; c > b; b++) a = a && j[b].isValid();
                        return a
                    },
                    any: function() {
                        for (var a = 0, b = j.length; b > a; a++)
                            if (j[a].isValid()) return !0;
                        return !1
                    },
                    none: function() {
                        for (var a = !0, b = 0, c = j.length; c > b; b++) a = a && !j[b].isValid();
                ...
                m = function() {
                    j = [];
                    for (var a = 0, b = e.rules.length; b > a; a++) {
                        var d = new c(e.rules[a]);
                        d.disabled ? console.log("ignore disabled rule: ", e.rules[a]) : j.push(d)
                    }
                },
                n = function() {
                    if (e.disabled) return console.log("ignore disabled logic"), !0;
                    for (var a = 0, b = j.length; b > a; a++) j[a].validate();
                    o()
                },
                o = function() {
                    f = k[e.match](), console.log("doAction: " + f), f && (console.log("going to " + e.action + " " + e.selector), l[e.action]())
                };
            m(), this.validate = n, this.isValid = f
        },
        e = function(a) {
